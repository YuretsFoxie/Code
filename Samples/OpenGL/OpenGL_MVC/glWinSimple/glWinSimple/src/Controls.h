...
// ListBox      : list box
// ListView     : ListView control, required comctl32.dll
...
// Tab          : tab control, required comctl32.dll
//
...
namespace Win
{
    ...
    ///////////////////////////////////////////////////////////////////////////
    // ListView class
    // It requires loading comctl32.dll.
    ///////////////////////////////////////////////////////////////////////////
    class ListView: public ControlBase
    {
    public:
        ListView() : ControlBase() {}
        ListView(HWND parent, int id, bool visible=true) : ControlBase(parent, id, visible) {}
        ~ListView() {}

        // insert item, LVITEM.iItem contains the index of the item
        // it returns the index of new item, otherwise -1
        int insertItem(LVITEM* item) { return (int)SendMessage(handle, LVM_INSERTITEM, 0, (LPARAM)item); }

        // set item's attributes, use to set subitems
        void setItem(LVITEM* item) { SendMessage(handle, LVM_SETITEM, 0, (LPARAM)item); }

        // get the number of items in the listview
        int getItemCount() { return (int)SendMessage(handle, LVM_GETITEMCOUNT, 0, 0); }

        // delete a selected item or all items
        void deleteItem(int index) { SendMessage(handle, LVM_DELETEITEM, index, 0); }
        void deleteItems() { SendMessage(handle,LVM_DELETEALLITEMS, 0, 0); }

        // insert a column to the listview. It return the index of new column
        int insertColumn(int index, LVCOLUMN* column) { return (int)SendMessage(handle, LVM_INSERTCOLUMN, index, (LPARAM)column); }

        // set/get image list (MODE: LVSIL_NORMAL, LVSIL_STATE)
        void setImageList(HIMAGELIST imageListHandle, int imageListType=LVSIL_NORMAL) const
                                                    { SendMessage(handle, LVM_SETIMAGELIST, (WPARAM)imageListType, (LPARAM)imageListHandle); }
        HIMAGELIST getImageList(int imageListType) const
                                                    { return (HIMAGELIST)SendMessage(handle, LVM_GETIMAGELIST, (WPARAM)imageListType, (LPARAM)0); }

        // return the handle of EditBox control of an item where LVN_BEGINLABELEDIT is notified
        HWND getEditControl() const                 { return (HWND)SendMessage(handle, LVM_GETEDITCONTROL, 0, 0); }

        // set view style: details, icon, list, smallicon, tile
        void setView(DWORD view) { SendMessage(handle, LVM_SETVIEW, view, 0); }

        // set ListView styles: LVS_EX_FULLROWSELECT,   LVS_EX_GRIDLINES,
        //                      LVS_EX_HIDELABELS,      LVS_EX_INFOTIP, etc
        // (See more styles in Extented List-View Styles)
        void setExtendedListViewStyle(DWORD styles) { SendMessage(handle, LVM_SETEXTENDEDLISTVIEWSTYLE, styles, styles); }
    ...
    public:
        ...
        // get text at the current index
        int getTextLength(int index) { if(index < 0 || index >= getCount()) return 0;
                                       (int)SendMessage(handle, CB_GETLBTEXTLEN, index, 0);
                                     }
        void getText(int index, wchar_t* buf) { if(index < 0 || index >= getCount()) return;
                                                SendMessage(handle, CB_GETLBTEXT, index, (LPARAM)buf);
                                              }
    ...
    public:
        Tab() : ControlBase() {}
        Tab(HWND parent, int id, bool visible=true) : ControlBase(parent, id, visible) {}
        ~Tab() {}

        // calculate rect of window (true), or display area (false)
        void adjustRect(bool forWindow, RECT* rect) const   { SendMessage(handle, TCM_ADJUSTRECT, (WPARAM)forWindow, (LPARAM)rect); }

        // delete item/items from tab
        void deleteItem(int index) const                    { SendMessage(handle, TCM_DELETEITEM, (WPARAM)index, 0); }
        void deleteAllItems() const                         { SendMessage(handle, TCM_DELETEALLITEMS, 0, 0); }

        // clear the state of all tabs if flag=true, otherwise clear all except the current
        void deselectAll(bool exceptCurr=false) const       { SendMessage(handle, TCM_DESELECTALL, (WPARAM)exceptCurr, 0); }

        // set/get focus index
        void setCurSel(int index) const                     { SendMessage(handle, TCM_SETCURSEL, (WPARAM)index, 0); }
        int  getCurSel() const                              { return (int)SendMessage(handle, TCM_GETCURSEL, 0, 0); }
        void setCurFocus(int index) const                   { SendMessage(handle, TCM_SETCURFOCUS, (WPARAM)index, 0); }
        int  getCurFocus() const                            { return (int)SendMessage(handle, TCM_GETCURFOCUS, 0, 0); }
        void highlightItem(int index, bool flag=true) const { SendMessage(handle, TCM_HIGHLIGHTITEM, (WPARAM)index, (LPARAM)flag); }

        // set/get item's attributes, pass ptr to TCITEM
        // NOTE: must set mask to get info
        void setItem(int index, TCITEM* item) const         { SendMessage(handle, TCM_SETITEM, (WPARAM)index, (LPARAM)item); }
        void getItem(int index, TCITEM* item) const         { SendMessage(handle, TCM_GETITEM, (WPARAM)index, (LPARAM)item); }
        int  getItemCount() const                           { return (int)SendMessage(handle, TCM_GETITEMCOUNT, 0, 0); }

        // set width/height of tabs
        void setItemSize(int width, int height) const       { SendMessage(handle, TCM_SETITEMSIZE, 0, MAKELPARAM(width, height)); }
        // get rect of selected tab
        void getItemRect(int index, RECT* rect) const       { SendMessage(handle, TCM_GETITEMRECT, (WPARAM)index, (LPARAM)rect); }
        int  setMinTabWidth(int width) const                { return (int)SendMessage(handle, TCM_SETMINTABWIDTH, 0, (LPARAM)width); } // return prev width

        // insert new item, return the index of tab item, if failed, return -1
        int insertItem(int index, const wchar_t* str, int imageIndex=-1) const
                {   // construct TCITEM struct
                    TCITEM tabItem;
                    tabItem.mask = TCIF_TEXT | TCIF_IMAGE;
                    tabItem.dwState = 0;
                    tabItem.dwStateMask = 0;
                    tabItem.pszText = (LPWSTR)str;
                    tabItem.cchTextMax = sizeof(str) / sizeof(str[0]);
                    tabItem.iImage = imageIndex;
                    tabItem.lParam = 0; // app-defined data
                    int itemIndex = (int)SendMessage(handle, TCM_INSERTITEM, (WPARAM)index, (LPARAM)&tabItem);
                    return itemIndex; // if success return same index, otherwise return -1
                }

        // set/get image list
        void setImageList(HIMAGELIST imageListHandle) const { SendMessage(handle, TCM_SETIMAGELIST, 0, (LPARAM)imageListHandle); }
        HIMAGELIST getImageList() const                     { return (HIMAGELIST)SendMessage(handle, TCM_GETIMAGELIST, 0, 0); }
        void removeImage(int index) const                   { SendMessage(handle, TCM_REMOVEIMAGE, (WPARAM)index, 0); }
    };
}
#endif
