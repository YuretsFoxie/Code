...
*/
    ...
    case WM_SETCURSOR:
        returnValue = ctrl->setCursor((HWND)wParam, LOWORD(lParam), HIWORD(lParam));     // handle, hit-test, ID of mouse event
...
INT_PTR CALLBACK Win::dialogProcedure(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONDOWN:
        ctrl->lButtonDown(wParam, LOWORD(lParam), HIWORD(lParam));              // state, x, y
        return true;
    ...
    case WM_CTLCOLORBTN:                                                        // hdc, hwnd (MUST return handle to brush)
        return ctrl->ctlColorBtn((HDC)wParam, (HWND)lParam);

    case WM_CTLCOLORDLG:                                                        // hdc, handle (MUST return handle to brush)
        return ctrl->ctlColorDlg((HDC)wParam, (HWND)lParam);

    case WM_CTLCOLOREDIT:                                                       // hdc, hwnd (MUST return handle to brush)
        return ctrl->ctlColorEdit((HDC)wParam, (HWND)lParam);

    case WM_CTLCOLORLISTBOX:                                                    // hdc, hwnd (MUST return handle to brush)
        return ctrl->ctlColorListBox((HDC)wParam, (HWND)lParam);

    case WM_CTLCOLORSCROLLBAR:                                                  // hdc, hwnd (MUST return handle to brush)
        return ctrl->ctlColorScrollBar((HDC)wParam, (HWND)lParam);

    case WM_CTLCOLORSTATIC:                                                     // hdc, hwnd (MUST return handle to brush)
        return ctrl->ctlColorStatic((HDC)wParam, (HWND)lParam);

    //case WM_SETCURSOR:
    //    ctrl->setCursor((HWND)wParam, LOWORD(lParam), HIWORD(lParam));          // handle, hit-test, ID of mouse event
    //    return true;
