...
LRESULT ControllerGL::close()
{
    ...
    Win::log("OpenGL window is closed.");
    return 0;
}
...
void ControllerGL::runThread()
{
    ...
    if(hBitmap)
    {
        ...
        // make sure bitmap mode is BI_RGB, otherwise it throws stack-based buffer overrun exception
        // NOTE: BI_BITFIELDS requires additional 3 DWORDs (3*32bit) in header to store bit masks
    ...
    // configure projection matrix with the window size
    RECT rect;
